...
@router.get("/identity/{identity_id}", response_model=IdentityInvestigationResponse)
def investigate_identity(identity_id: str):
    ...
    if scenario == "NORMAL":
        ...
        behaviour_profile = [
            BehaviouralSignal(name="Location consistency", value="Primary office location", status="normal"),
            BehaviouralSignal(name="Device consistency", value="Primary work device", status="normal"),
            BehaviouralSignal(name="Failed authentication attempts", value=f"{rng.randint(0, 2)} attempts", status="normal"),
            BehaviouralSignal(name="Login frequency", value=f"{rng.randint(1, 5)} per day", status="normal"),
            BehaviouralSignal(name="Resource access behaviour", value="Standard department resources", status="normal"),
        ]
        ...
        behaviour_profile = [
            BehaviouralSignal(name="Location consistency", value="Secondary office / Remote", status="deviation"),
            BehaviouralSignal(name="Device consistency", value="Primary work device", status="normal"),
            BehaviouralSignal(name="Failed authentication attempts", value=f"{rng.randint(3, 8)} attempts", status="deviation"),
            BehaviouralSignal(name="Login frequency", value=f"{rng.randint(2, 6)} per day", status="normal"),
            BehaviouralSignal(name="Resource access behaviour", value="Standard department resources", status="normal"),
        ]
        ...
        behaviour_profile = [
            BehaviouralSignal(name="Location consistency", value="Unrecognized IP / Country", status="deviation"),
            BehaviouralSignal(name="Device consistency", value="Unknown Device Type", status="deviation"),
            BehaviouralSignal(name="Failed authentication attempts", value=f"{rng.randint(15, 50)} attempts", status="deviation"),
            BehaviouralSignal(name="Login frequency", value="Outside business hours", status="deviation"),
            BehaviouralSignal(name="Resource access behaviour", value="Accessing sensitive resources", status="deviation"),
        ]
    ...
    # 4. Threat Fusion
    threat_fusion = round((anomaly_score * 0.4) + (transformer_score * 0.4) + (behaviour_deviation_score * 0.2), 1)
