...
class DashboardService:
    def __init__(self, alerts_path=None, reports_path=None):
        ...
        if reports_path is None:
            self.reports_path = PROJECT_ROOT / "reports" / \
                "predictions" / "deeptrace_threat_report.csv"
        else:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        critical = int((alerts_df['RiskLevel'] == 'Critical').sum(
        )) if 'RiskLevel' in alerts_df.columns else 0
        high = int((alerts_df['RiskLevel'] == 'High').sum()
                   ) if 'RiskLevel' in alerts_df.columns else 0
        medium = int((alerts_df['RiskLevel'] == 'Medium').sum(
        )) if 'RiskLevel' in alerts_df.columns else 0

        risk_dist = alerts_df['RiskLevel'].value_counts(
        ).to_dict() if 'RiskLevel' in alerts_df.columns else {}
        severity_dist = alerts_df['Severity'].value_counts(
        ).to_dict() if 'Severity' in alerts_df.columns else {}
        status_dist = alerts_df['Status'].value_counts(
        ).to_dict() if 'Status' in alerts_df.columns else {}
